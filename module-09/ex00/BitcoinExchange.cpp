...
std::time_t BitcoinExchange::get_date(std::string input, std::string delimiter)
{
	...
	if (!str2nbr(year_int, year) || !str2nbr(month_int, month) || !str2nbr(day_int, day))
		throw DateIsInvalid();
...
{
	std::string value_str = input.substr(input.find_first_of(delimiter) + delimiter.length());
...
template<class T>
bool BitcoinExchange::str2nbr (T &i, std::string s)
{
    std::stringstream ss;
	ss << s;
    ss >> i;
...
std::string BitcoinExchange::get_date(const std::time_t & date)
{
	...
	date_str << (date_tm.tm_year + 1900) << "-" << std::setfill('0') << std::setw(2) << (date_tm.tm_mon + 1) << "-" << std::setfill('0') << std::setw(2) << date_tm.tm_mday;
...
void BitcoinExchange::process_input(std::ifstream & input)
{
	...
	while (std::getline(input, input_str))
	{
		...
		try
		{
			...
			std::cout << get_date(date) << " => " << value << " => " << get_exchange_rate(date) * value << std::endl;
		}
